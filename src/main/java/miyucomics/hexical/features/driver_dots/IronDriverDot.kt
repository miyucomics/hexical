...
object IronDriverDot : AbstractDriverDot() {
	...
	@JvmStatic
	fun handleIronDriverDot(vm: CastingVM, iota: PatternIota, continuation: SpellContinuation): CastResult? {
		val env = vm.env
		val pattern = iota.pattern
		if (!canTrigger(env, pattern, continuation))
			return null

		val program = (env.caster as ServerPlayerEntity).itemCache().ironDriverDotMacros[pattern.anglesSignature()] ?: return null
		return constructCastResult(vm, continuation, iota, program)
	}
}
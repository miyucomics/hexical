	...
	fun init() {
		...
		ItemTooltipCallback.EVENT.register { stack, context, lines ->
			...
			lines.add(Text.translatable("hexcasting.tooltip.media_amount.advanced",
				Text.literal(CharmedItemUtilities.DUST_AMOUNT.format((media / MediaConstants.DUST_UNIT.toFloat()).toDouble())).styled { style -> style.withColor(ItemMediaHolder.HEX_COLOR) },
				Text.translatable("hexcasting.tooltip.media", CharmedItemUtilities.DUST_AMOUNT.format((maxMedia / MediaConstants.DUST_UNIT.toFloat()).toDouble())).styled { style -> style.withColor(ItemMediaHolder.HEX_COLOR) },
				Text.literal(CharmedItemUtilities.PERCENTAGE.format((100f * media / maxMedia).toDouble()) + "%").styled { style -> style.withColor(TextColor.fromRgb(mediaBarColor(media, maxMedia))) }
			))
		}

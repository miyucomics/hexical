...
class OpGetBlockStateData(private val mode: Int) : ConstMediaAction {
	...
	override fun execute(args: List<Iota>, ctx: CastingContext): List<Iota> {
		...
		when (mode) {
			0 -> {
				state.entries[Properties.WATERLOGGED] ?: return listOf(NullIota())
				return state.get(Properties.WATERLOGGED).asActionResult
			}
			1 -> {
				return if (state.entries[Properties.FACING] != null) state.get(Properties.FACING).unitVector.asActionResult
			...
			2 -> {
				return if (state.entries[Properties.AGE_1] != null) (state.get(Properties.AGE_1)).asActionResult
			...
			3 -> {
				state.entries[Properties.LIT] ?: return listOf(NullIota())
				return state.get(Properties.LIT).asActionResult
			}
			4 -> {
				state.entries[Properties.OPEN] ?: return listOf(NullIota())
				return state.get(Properties.OPEN).asActionResult
			}
			5 -> {
				state.entries[Properties.ROTATION] ?: return listOf(NullIota())
				return state.get(Properties.ROTATION).asActionResult
			}
			6 -> {
				when (state.block) {
					is CandleBlock -> {
						state.entries[Properties.CANDLES] ?: return listOf(NullIota())
						return state.get(Properties.CANDLES).asActionResult
					}
					is SeaPickleBlock -> {
						state.entries[Properties.PICKLES] ?: return listOf(NullIota())
						return state.get(Properties.PICKLES).asActionResult
					}
					is TurtleEggBlock -> {
						state.entries[Properties.EGGS] ?: return listOf(NullIota())
						return state.get(Properties.EGGS).asActionResult
					}
					else -> return listOf(NullIota())
				}
			}
			7 -> {
				return if (state.entries[Properties.HAS_BOOK] != null) state.get(Properties.HAS_BOOK).asActionResult

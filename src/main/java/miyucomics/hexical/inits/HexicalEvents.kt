...
import miyucomics.hexical.features.player_state.fields.LesserSentinelField
	...
	fun init() {
		...
		CastingEnvironment.addCreateEventListener { env: CastingEnvironment, _: NbtCompound ->
			...
			if (LedgerField.isEnvCompatible(env))
				env.addExtension(LedgerRecordComponent(env as PlayerBasedCastEnv))
		}

		ServerPlayerEvents.AFTER_RESPAWN.register { old, new, alive ->
			if (!alive)
				ShaderRenderer.setEffect(null)
			new.getHexicalPlayerState().handleRespawn(new, old)
		}
	}

...
object HexicalActions {
	fun init() {
		...
		register("get_lamp_position", "qwddedqdd", HexDir.SOUTH_WEST, OpGetLampData { _, nbt -> vecFromNBT(nbt.getCompound("position")).asActionResult })
		register("get_lamp_rotation", "qwddedadw", HexDir.SOUTH_WEST, OpGetLampData { _, nbt -> vecFromNBT(nbt.getCompound("rotation")).asActionResult })
		register("get_lamp_velocity", "qwddedqew", HexDir.SOUTH_WEST, OpGetLampData { _, nbt -> vecFromNBT(nbt.getCompound("velocity")).asActionResult })
		register("get_lamp_use_time", "qwddedqwddwa", HexDir.SOUTH_WEST, OpGetLampData { env, nbt -> (env.world.time - (nbt.getDouble("start_time") + 1.0)).asActionResult })
		register("get_lamp_media", "qwddedaeeeee", HexDir.SOUTH_WEST, OpGetLampData { env, _ -> ((env.castingEntity!!.activeItem.item as HandLampItem).getMedia(env.castingEntity!!.activeItem).toDouble() / MediaConstants.DUST_UNIT).asActionResult })
		register("get_lamp_storage", "qwddedqwaqqqqq", HexDir.SOUTH_WEST, OpGetLampData { env, nbt -> listOf(IotaType.deserialize(nbt.getCompound("storage"), env.world)) })
		register("set_lamp_storage", "qwddedqedeeeee", HexDir.SOUTH_WEST, OpSetLampStorage)
